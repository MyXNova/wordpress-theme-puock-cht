...
class PuockUserCenter implements IPuockClassLoad
{
    ...
    public static function update_profile()
    {
        if (is_string($data = pk_get_req_data([
                'nickname' => ['name' => __('昵稱', PUOCK), 'required' => true, 'remove_html' => true],
                'user_url' => ['name' => __('網站地址', PUOCK), 'remove_html' => true, 'empty' => true],
                'description' => ['name' => __('個人說明', PUOCK), 'remove_html' => true, 'empty' => true],
            ])) === true) {
        ...
        if (!$user) {
            echo pk_ajax_resp_error(__('請先登入', PUOCK));
            wp_die();
        }
        $data['ID'] = $user->ID;
        ...
        if (!wp_update_user($data)) {
            echo pk_ajax_resp_error(__('儲存失敗', PUOCK));
            wp_die();
        }
        echo pk_ajax_resp(null, __('儲存成功', PUOCK));
    ...
    public static function page_profile()
    {
        ...
        <form action="<?php echo pk_ajax_url('pk_user_update_profile') ?>" class="ajax-form" data-no-reset>
                ...
                <div class="col-sm-10">
                    <div class="row align-items-center">
                        ...
                        <div class="col-8 d-flex">
                            ...
                            <?php endif; ?>
                            <button type="button"
                                    onclick="layer.confirm('<?php _e('確認註銷登入嗎？', PUOCK) ?>',function (){window.Puock.goUrl('<?php echo wp_logout_url('/'); ?>')})"
                                    class="btn btn-ssm btn-danger">
                                <i class="fa fa-sign-out"></i>
                                <span><?php _e('註銷登入', PUOCK) ?></span>
                            </button>
                        </div>
                    </div>
                </div>
            ...
            <div class="mb-3 row">
                <label class="col-sm-2 col-form-label"><?php _e('使用者名稱', PUOCK) ?></label>
                <div class="col-sm-10">
                    <input type="text" readonly class="form-control" value="<?php echo $userinfo->user_nicename ?>">
                    <small class="c-sub"><?php _e('使用者名稱不可更改', PUOCK) ?></small>
                </div>
            </div>
            <div class="mb-3 row">
                <label class="col-sm-2 col-form-label"><?php _e('昵稱', PUOCK) ?></label>
                <div class="col-sm-10">
                    <input name="nickname" type="text" class="form-control" value="<?php echo $userinfo->nickname ?>">
                </div>
            </div>
            <div class="mb-3 row">
                <label class="col-sm-2 col-form-label"><?php _e('網站地址', PUOCK) ?></label>
                <div class="col-sm-10">
                    <input name="user_url" type="url" class="form-control" value="<?php echo $userinfo->user_url ?>">
                </div>
            </div>
            <div class="mb-3 row">
                <label class="col-sm-2 col-form-label"><?php _e('個人說明', PUOCK) ?></label>
                <div class="col-sm-10">
